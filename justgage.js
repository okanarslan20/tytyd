...
JustGage = function(config) {
  ...
  if (obj.config.donut === true) {
    ...
    // value
    valueFontSize = ((widgetH / 6.4) > 16) ? (widgetH / 9) : 18;
    valueX = dx + widgetW / 2;
    if (obj.config.label !== '') {
  ...
  if (obj.config.textRenderer) {
    ...
    obj.originalValue = (obj.originalValue * 1).toFixed(2) + obj.config.symbol;
  }
  ...
  if (obj.config.counter === true) {
    ...
    eve.on("raphael.anim.frame." + (obj.level.id), function() {
      ...
      if (obj.config.textRenderer) {
        ...
        obj.txtValue.attr("text", (currentValue * 1).toFixed(2) + obj.config.symbol);
      }
      setDy(obj.txtValue, obj.params.valueFontSize, obj.params.valueY);
      currentValue = null;
    });
...
JustGage.prototype.refresh = function(val, max) {
  ...
  if (obj.config.textRenderer) {
    ...
    displayVal = formatNumber((displayVal * 1).toFixed(2)) + obj.config.symbol;
  } else {
    displayVal = (displayVal * 1).toFixed(2) + obj.config.symbol;
  }
